<?php
    ...
    $name_0 = !empty($_GET['name_0']) ? $_GET['name_0'] : 'Batman';
    $name_1 = !empty($_GET['name_1']) ? $_GET['name_1'] : 'Arrow';



    $heroes_0 = file_get_contents('http://superheroapi.com/api/2024501571102531/search/'.$name_0.'');
    $heroes_0 = json_decode($heroes_0);


    $heroes_1 = file_get_contents('http://superheroapi.com/api/2024501571102531/search/'.$name_1.'');
    ...
    // $url = 'http://superheroapi.com/api/2024501571102531/400/biography';
...
<body>
    ...
    <form action="#" method="get">
        <input class='searchBar' type="text" value="<?= $name_0; ?>" name="name_0" onkeyup="showResult(this.value)">
        <div class="livesearch"></div>
        <div>VS</div>
        <input class='searchBar' type="text" value="<?= $name_1; ?>" name="name_1" onkeyup="showResult(this.value)">
        <div class="livesearch"></div>
        <input type="submit">
    </form>
    ...
    <!-- <form class='search'>
        ...
        <div class="heroes">
            <div><?=$name_0?></div>
            <div>Name: <?= $heroes_0->results[0]->name; ?></div>
            <div>Full-name: <?= $heroes_0->results[0]->biography->{'full-name'} ?></div>                
            <div>Intelligence: <?= $heroes_0->results[0]->powerstats->intelligence?></div>
            <img src="<?= str_replace('http://', 'https://', $heroes_0->results[0]->image->url)?>" width="240px" height="320px" alt="">

        </div>

        <div class="heroes">
            <div><?=$name_1?></div>
            <div>Name: <?= $heroes_1->results[0]->name; ?></div>
            <div>Full-name: <?= $heroes_1->results[0]->biography->{'full-name'} ?></div>
            <div>Intelligence: <?= $heroes_1->results[0]->powerstats->intelligence?></div>
            <img src="<?= str_replace('http://', 'https://', $heroes_1->results[0]->image->url)?>" width="240px" height="320px" alt="">
                
        </div>

        <div>
            <?php 
                if (($heroes_0->results[0]->powerstats->intelligence) > ($heroes_1->results[0]->powerstats->intelligence))
                {
                    $clever= $heroes_0->results[0]->name;
                    $message_result= $clever.' est plus intelligent que '.$heroes_1->results[0]->name ;

                }   
                else if (($heroes_0->results[0]->powerstats->intelligence)<($heroes_1->results[0]->powerstats->intelligence))
                {
                    $clever= $heroes_1->results[0]->name;
                    $message_result= $clever.' est plus intelligent que '.$heroes_0->results[0]->name;
                }
                else
                {
                    $clever="Ils possèdent la même intelligence";
                    $message_result=$clever;
                }
